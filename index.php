...
$sql=$sql."(SELECT bid_amount FROM bid WHERE lot_id=L.id ORDER BY bid_date DESC LIMIT 1)";
$sql=$sql."last_bid FROM lot L JOIN category C ON L.category_id = C.id WHERE ";
$sql=$sql."expiration_date >= CURRENT_TIMESTAMP ORDER BY creation_date DESC  LIMIT 9";
$result = mysqli_query($con, $sql);
$goods = mysqli_fetch_all($result, MYSQLI_ASSOC);

foreach($goods as $num => $row)
{
    $row['last_bid'] == NULL ? $goods[$num]['price']=$row['start_price'] : $goods[$num]['price']=$row['last_bid'];
}
